...
internal class ProxyActivity : AppCompatActivity() {
    ...
    private val clientIntent by lazy {
        intent.getParcelableExtra<Intent>(Proxy.KEY_INTENT)
            ?: throw exception("Intent cannot be null.")
    }
    private val resultReceiver by lazy {
        intent.getParcelableExtra<ResultReceiver>(Proxy.KEY_RESULT_RECEIVER)
            ?: throw exception("ResultReceiver cannot be null.")
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (clientIntent.resolveActivity(packageManager) == null) {
            ...
            startActivityForResult(
                clientIntent, intent.getIntExtra(
                    Proxy.KEY_REQUEST_CODE,
                    Proxy.DEFAULT_REQUEST_CODE
                )
            )
        }
    }
