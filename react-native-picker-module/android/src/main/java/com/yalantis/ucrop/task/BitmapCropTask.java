...
public class BitmapCropTask extends AsyncTask<Void, Void, Throwable> {
    ...
    private final int mCompressionMaxSizeBytes;
    ...
    public BitmapCropTask(
        ...
        mCompressionMaxSizeBytes = cropParameters.getmCompressionMaxSizeBytes();
    ...
    private void saveImage(@NonNull Bitmap croppedBitmap) throws FileNotFoundException {
        ...
        if(mCompressionMaxSizeBytes == 0) {
            ...
            do {
                outputStream = context.getContentResolver().openOutputStream(Uri.fromFile(new File(mImageOutputPath)));
                croppedBitmap.compress(Bitmap.CompressFormat.JPEG, currQuality, outputStream);
                currSize = stream.toByteArray().length;
                // limit quality by 5 percent every time
                currQuality -= 5;
            } while (currSize >= mCompressionMaxSizeBytes);
            croppedBitmap.recycle();
            BitmapLoadUtils.close(outputStream);
        }
    }
