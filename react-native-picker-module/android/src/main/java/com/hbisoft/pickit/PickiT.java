...
public class PickiT implements CallBackTask{
        ...
        if (APILevel>=19){
            ...
            // Local file was selected
                ...
                if (returnedPath == null || returnedPath.equals("")){
                    ...
                    pickiTCallbacks.PickiTonCompleteListener(returnedPath, false, false, false, "", 0, Utils.errorReason());
                }
                // Path is not null
                else {
                    // This can be caused by two situations
                    ...
                    int originalFileSize = ((int) new File(returnedPath).length());
                    // Path can be returned, no need to make a "copy"
                    pickiTCallbacks.PickiTonCompleteListener(returnedPath, false, false, true, returnedPath.substring(returnedPath.lastIndexOf("/") + 1), originalFileSize, "");
                }
            }
        }else{
            //Todo: Test API <19
            returnedPath = Utils.getRealPathFromURI_BelowAPI19(context, uri);
            pickiTCallbacks.PickiTonCompleteListener(returnedPath, false, false, true, "API <19", 0, "");
        }
    ...
    @Override
    public void PickiTonPostExecute(
            String path,
        ...
        if (wasSuccessful) {
            if (isDriveFile) {
                pickiTCallbacks.PickiTonCompleteListener(path, true, false, true, originalFileName, originalFileSize, "");
            } else if (isFromUnknownProvider) {
                pickiTCallbacks.PickiTonCompleteListener(path, false, true, true, originalFileName, originalFileSize, "");
            }
        }else {
            if (isDriveFile) {
                pickiTCallbacks.PickiTonCompleteListener(path, true, false, false, originalFileName, originalFileSize, reason);
            } else if (isFromUnknownProvider) {
                pickiTCallbacks.PickiTonCompleteListener(path, false, true, false, originalFileName, originalFileSize, reason);
            }
        }
    }
